function [pol,foil,err] = xfoil(coord,alpha,Re,Mach,varargin)
...
fid = fopen([wd filesep fname '.inp'],'wt');
fprintf(fid,'plop\ng\n\n\n',coord(5:end));
if (fid<=0),
...
% Polar output filename
 ...
 % execute xfoil
  ...
  if (fid<=0),
    ...
    % data
	do
		buff=fscanf(fid,'%c',[1,1]);
	until(buff(1,1)=='-')
	buff=fgetl(fid);
	P{1}=(fscanf(fid,'%f',[7,length(alpha)]))';
	fclose(fid);
   delete([wd filesep file_pwrt]);
    % ‚à‚µP‚Ì’l‚ª‚È‚¢‚Ì‚È‚çƒGƒ‰[‚ð‚Í‚­
	if length(P{1})==0;
		err=1;
		pol=0;
		foil=0;
