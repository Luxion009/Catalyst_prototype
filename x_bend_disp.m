...
%disp("generating Xx");
for n=1:div_c
	Xx(dp_num*(n-1)+1:dp_num*n,1)=S_I(dp_num*(n-1)+1:dp_num*n,1)./ply_data{n}(rows(ply_data{n}),1).*1000;%断面係数
end

for n=1:div_c
	S_max_X(1:dp_num*n,1)=S_matDM(1:dp_num*n,1)./Xx(1:dp_num*n,1).*9.8./1000;%最大曲げ応力

end

for i=1:div_c*dp_num-1
	S_mat_dDtheta(i,1)=(S_matDM(i,1)/S_H(i,1)+S_matDM(i+1,1)/S_H(i+1,1))/2*(S_mat(i+1,1)-S_mat(i,1));%連続する二つの曲率の平均にその間の距離をかける
	%接線の角度変化
end
S_mat_Dtheta(1,1)=0;%接線と水平軸の角度
...
for i=1:div_c*dp_num-1
	S_mat_dDswang(i,1)=(S_mat_Dtheta(i,1)+S_mat_Dtheta(i+1,1))./2.*(S_mat(i+1,1)-S_mat(i,1));%sin x=xと近似できるのでX方向変化が算出できる
end
S_mat_Dswang(1,1)=S_mat_dDswang(1,1);
for i=2:div_c*dp_num-1
	S_mat_Dswang(i,1)=S_mat_Dswang(i-1,1)+S_mat_dDswang(i,1);%X方向位置
end
