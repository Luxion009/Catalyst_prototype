clear lst_ydata lst_fdata lst_re lst_gamma
...
%ヨー核速度にあわせてレイノルズ数を変化させる
lst_re(:,1)=(d_vel+r_avel.*lst_fdata(:,1)).*lst_fdata(:,2)./nu;
...
disp("calculating comp_vel");
for i=1:(wld_c+we_div)
	lst_comp_vel(i,1)=sqrt((d_vel+r_avel.*lst_fdata(i,1))^2+lst_Vn_i(i,1)^2);
endfor
...
%力をモーメントに
M_ir(1,1)=trapz(lst_fdata(:,1),dM_ir(:,1).*lst_fdata(:,1));
M_ir(2,1)=trapz(lst_fdata(:,1),dM_ir(:,2).*lst_fdata(:,1));

M_or(1,1)=trapz(lst_fdata(:,1),dM_or(:,1).*lst_fdata(:,1));
M_or(2,1)=trapz(lst_fdata(:,1),dM_or(:,2).*lst_fdata(:,1));

%復元モーメント
r_m=M_ir(1,1)-M_or(1,1);
%ヨーダンピング
r_y=M_ir(2,1)-M_or(2,1);


Clr=r_m/r_avel/(Kp*span^2/(2*d_vel))
Cnr=r_y/r_avel/(Kp*span^2/(2*d_vel))