...
defad_vel=input("解析機速(m/s)=");
ang_adj=input("増減取り付け角=");
...
do
	...
	c++;
		...
		for i=1:(wld_c+we_div)
			lst_i_ang(i,1)=atan(lst_Vn_i(i,1)/defad_vel)*180/pi;
		endfor

		%合成速度計算
		disp("calculating comp_vel");
		for i=1:(wld_c+we_div)
			lst_comp_vel(i,1)=sqrt(defad_vel^2+lst_Vn_i(i,1)^2);
		endfor
		...
		for n=1:we_div+wld_c
			...
			[bufx foil err]=xfoil([x_foil lst_ydata(:,n)],f_mtang(n,1)-lst_i_ang(n,1)+ang_adj,lst_re(n,1),0);
			if(err==0)
		...
		if errc!=1
			for n=1:errc-1
				ncc=errlist(n,1)
				lst_data(ncc,:)=[interp1(nang_cp(ncc-1:2:ncc+1,1),lst_data(ncc-1:2:ncc+1,1),nang_cp(ncc,1),"spline") interp1(nang_cp(ncc-1:2:ncc+1,1),lst_data(ncc-1:2:ncc+1,2),nang_cp(ncc,1),"spline") interp1(nang_cp(ncc-1:2:ncc+1,1),lst_data(ncc-1:2:ncc+1,3),nang_cp(ncc,1),"spline")];
			endfor
		endif
		buff=input("")
		...
		disp(Di_lst);
		D_lst=Dp_lst+Di_lst
		W_LST=(D_lst+6)*defad_vel/0.85/0.9
		c
