do
	...
	do
		...
		do
			...
			for i=1:w_div_c
				for j=1:dw_divn
					ddds=d_s(i,1)/2;
					line_e(dw_divn*(i-1)+j,1)=newton_met(a,alpha,ddds,f_v);
					a=line_e(dw_divn*(i-1)+j,1);
					f_v=a;
				endfor
			endfor
			%分割翼ごとに分割幅が異なる
		...
		d_count=1;
		do
			nang_cp(d_count,1)=line_e_d(d_count,1)/2+sum(line_e_d(1:d_count-1));
			d_count=d_count+1;
		...
		%上半を考慮しなかった時の線素端位置を求める

		nang_line(1,1)=line_e_d(1,1);
		for i=2:we_div
			nang_line(i,1)=line_e_d(i,1)+sum(line_e_d(1:i-1));

		endfor
		
		if(wl_span>0)
			nang_line(we_div+1,1)=nang_line(we_div,1)+wl_d;
			for i=(we_div+2):(we_div+wld_c)
				nang_line(i,1)=nang_line(i-1)+wl_d;
			endfor
		endif
		...
		figure(30)
			plot(nang_cp(:,1),Op_gamma(:,1),nang_cp(:,1),d_gamma(:,1));
			xlabel('y[m]');
			ylabel('gamma');
	...
	%%%試験のため循環分布を多角形近似する
	for i=1:dw_divn
		Hex_gamma(i,1)=(Op_gamma(dw_divn+1,1)+Op_gamma(dw_divn,1))/2;
	endfor

	for i=2:w_div_c
		gamma_d(i,1)=((Op_gamma(i*dw_divn,1)+Op_gamma(i*dw_divn+1,1))/2-(Op_gamma((i-1)*dw_divn,1)+Op_gamma((i-1)*dw_divn+1,1))/2)/dw_divn;
		Hex_gamma((i-1)*dw_divn+1,1)=(Op_gamma((i-1)*dw_divn,1)+Op_gamma((i-1)*dw_divn+1,1))/2+gamma_d(i,1)/2;
		for j=2:dw_divn
			Hex_gamma((i-1)*dw_divn+j,1)=Hex_gamma((i-1)*dw_divn+j-1,1)+gamma_d(i,1);
		endfor
	endfor

	Hex_gamma(we_div+1:we_div+wld_c,1)=spline(nang_cp(:,1),Op_gamma(:,1),nang_line(we_div+1:we_div+wld_c,1));

	%%設計用にコントロールポイントで定義されたHex_gammaを線素端で定義し直す

	for i=1:dw_divn
		ed_Hex_gamma(i,1)=(Op_gamma(dw_divn+1,1)+Op_gamma(dw_divn,1))/2;
	endfor

	for i=2:w_div_c
		ed_gamma_d(i,1)=((Op_gamma(i*dw_divn,1)+Op_gamma(i*dw_divn+1,1))/2-(Op_gamma((i-1)*dw_divn,1)+Op_gamma((i-1)*dw_divn+1,1))/2)/dw_divn;
		ed_Hex_gamma((i-1)*dw_divn+1,1)=(Op_gamma((i-1)*dw_divn,1)+Op_gamma((i-1)*dw_divn+1,1))/2+ed_gamma_d(i,1);
		for j=2:dw_divn
			ed_Hex_gamma((i-1)*dw_divn+j,1)=ed_Hex_gamma((i-1)*dw_divn+j-1,1)+ed_gamma_d(i,1);
		endfor
	endfor

	ed_Hex_gamma(we_div+1:we_div+wld_c,1)=Hex_gamma(we_div+1:we_div+wld_c,1);
	...
	disp("calculating Hex_L.");
	for i=1:(wld_c+we_div)
		Hex_dd_l(i,1)=Hex_gamma(i,1)*cos(line_e_ang(i,1))*line_e_d(i,1)/2;
	endfor
	Hex_re_def_l=4*a_density*d_vel*sum(Hex_dd_l(:,1));

	%設計用に線素端で線形に近似された誘導速度とかを計算する
	disp("calculating ed_Hex_Vn_i.");

	poly_e=polyfit(nang_cp(1:we_div,1),Hex_Vn_i(1:we_div,1),1);

	for i=2:w_div_c

		for j=1:dw_divn
			ed_Hex_Vn_i((i-1)*dw_divn+j,1)=poly_e(1).*nang_line((i-1)*dw_divn+j,1).+poly_e(2);
		endfor
	endfor
	...
	%誘導角度
	disp("calculating ed_Hex_i_ang.");
	for i=1:(wld_c+we_div)
		ed_Hex_i_ang(i,1)=atan(ed_Hex_Vn_i(i,1)/d_vel)*180/pi;
	endfor
	
	%合成速度計算
	disp("calculating Hex_comp_vel");
	for i=1:(wld_c+we_div)
		ed_Hex_comp_vel(i,1)=sqrt(d_vel^2+ed_Hex_Vn_i(i,1)^2);
	endfor
	...
	for i=2:w_div_c
		bufg=(ed_Hex_gamma(i*dw_divn,1)-ed_Hex_gamma((i-1)*dw_divn,1))/(nang_line(i*dw_divn,1)-nang_line((i-1)*dw_divn,1));
		printf("ed_gamma_d(%d,1)=%f\n",i,bufg);
	...
	figure(2);
		...
		% subplot(2,1,1);
			plot(nang_cp(:,1),Op_gamma(:,1),"-",nang_cp(:,1),Hex_gamma(:,1),"-");
			xlabel('y[m]');
			ylabel('gamma');
			xlim([0,span/2+wl_span+1]);
			grid on;
	...
	figure(5);
		subplot(2,1,1);
			plot(nang_cp(:,1),Hex_Vn_i(:,1),"-",nang_line(:,1),ed_Hex_Vn_i(:,1),"-");
			xlabel('y[m]');
			ylabel('Vn_i[m/s]');
			xlim([0,span/2+wl_span+1]);
			grid on;
		...
		subplot(2,1,2);
			plot(nang_cp(:,1),Hex_i_ang(:,1),"-",nang_line(:,1),ed_Hex_i_ang(:,1),"-");
			xlabel('y[m]');
			ylabel('i_angle');
			xlim([0,span/2+wl_span+1]);
			grid on;
